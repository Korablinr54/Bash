...
▒, милый для меня.
```
Ключ `f` позволит смотреть файл в режиме реального времени, напрмиер можно открыть терминал в отдельном окне и запустить файл для отслеживания например логов.

# nano
Удобный минималистичный редактор, который запускается `nano <file name>`.  
`горячие клавиши`:  
...
| Ctrl + W          | Поиск текста                  |
| Alt + W           | Повтор предыдущего поиска     |
| Ctrl + Y          | Прокрутка страницы вверх      |
| Ctrl + V          | Прокрутка страницы вниз       |
| Ctrl + _          | Переход к конкретной строке   |